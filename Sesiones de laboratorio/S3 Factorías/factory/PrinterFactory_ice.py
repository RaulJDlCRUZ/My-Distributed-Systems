# -*- coding: utf-8 -*-
#
# Copyright (c) ZeroC, Inc. All rights reserved.
#
#
# Ice version 3.7.9
#
# <auto-generated>
#
# Generated from file `PrinterFactory.ice'
#
# Warning: do not edit this file.
#
# </auto-generated>
#

from sys import version_info as _version_info_
import Ice, IcePy

# Start of module Example
_M_Example = Ice.openModule('Example')
__name__ = 'Example'

_M_Example._t_Printer = IcePy.defineValue('::Example::Printer', Ice.Value, -1, (), False, True, None, ())

if 'PrinterPrx' not in _M_Example.__dict__:
    _M_Example.PrinterPrx = Ice.createTempClass()
    class PrinterPrx(Ice.ObjectPrx):

        def write(self, message, context=None):
            return _M_Example.Printer._op_write.invoke(self, ((message, ), context))

        def writeAsync(self, message, context=None):
            return _M_Example.Printer._op_write.invokeAsync(self, ((message, ), context))

        def begin_write(self, message, _response=None, _ex=None, _sent=None, context=None):
            return _M_Example.Printer._op_write.begin(self, ((message, ), _response, _ex, _sent, context))

        def end_write(self, _r):
            return _M_Example.Printer._op_write.end(self, _r)

        def destroy(self, context=None):
            return _M_Example.Printer._op_destroy.invoke(self, ((), context))

        def destroyAsync(self, context=None):
            return _M_Example.Printer._op_destroy.invokeAsync(self, ((), context))

        def begin_destroy(self, _response=None, _ex=None, _sent=None, context=None):
            return _M_Example.Printer._op_destroy.begin(self, ((), _response, _ex, _sent, context))

        def end_destroy(self, _r):
            return _M_Example.Printer._op_destroy.end(self, _r)

        @staticmethod
        def checkedCast(proxy, facetOrContext=None, context=None):
            return _M_Example.PrinterPrx.ice_checkedCast(proxy, '::Example::Printer', facetOrContext, context)

        @staticmethod
        def uncheckedCast(proxy, facet=None):
            return _M_Example.PrinterPrx.ice_uncheckedCast(proxy, facet)

        @staticmethod
        def ice_staticId():
            return '::Example::Printer'
    _M_Example._t_PrinterPrx = IcePy.defineProxy('::Example::Printer', PrinterPrx)

    _M_Example.PrinterPrx = PrinterPrx
    del PrinterPrx

    _M_Example.Printer = Ice.createTempClass()
    class Printer(Ice.Object):

        def ice_ids(self, current=None):
            return ('::Example::Printer', '::Ice::Object')

        def ice_id(self, current=None):
            return '::Example::Printer'

        @staticmethod
        def ice_staticId():
            return '::Example::Printer'

        def write(self, message, current=None):
            raise NotImplementedError("servant method 'write' not implemented")

        def destroy(self, current=None):
            raise NotImplementedError("servant method 'destroy' not implemented")

        def __str__(self):
            return IcePy.stringify(self, _M_Example._t_PrinterDisp)

        __repr__ = __str__

    _M_Example._t_PrinterDisp = IcePy.defineClass('::Example::Printer', Printer, (), None, ())
    Printer._ice_type = _M_Example._t_PrinterDisp

    Printer._op_write = IcePy.Operation('write', Ice.OperationMode.Normal, Ice.OperationMode.Normal, False, None, (), (((), IcePy._t_string, False, 0),), (), None, ())
    Printer._op_destroy = IcePy.Operation('destroy', Ice.OperationMode.Normal, Ice.OperationMode.Normal, False, None, (), (), (), None, ())

    _M_Example.Printer = Printer
    del Printer

_M_Example._t_PrinterFactory = IcePy.defineValue('::Example::PrinterFactory', Ice.Value, -1, (), False, True, None, ())

if 'PrinterFactoryPrx' not in _M_Example.__dict__:
    _M_Example.PrinterFactoryPrx = Ice.createTempClass()
    class PrinterFactoryPrx(Ice.ObjectPrx):

        def create(self, name, context=None):
            return _M_Example.PrinterFactory._op_create.invoke(self, ((name, ), context))

        def createAsync(self, name, context=None):
            return _M_Example.PrinterFactory._op_create.invokeAsync(self, ((name, ), context))

        def begin_create(self, name, _response=None, _ex=None, _sent=None, context=None):
            return _M_Example.PrinterFactory._op_create.begin(self, ((name, ), _response, _ex, _sent, context))

        def end_create(self, _r):
            return _M_Example.PrinterFactory._op_create.end(self, _r)

        @staticmethod
        def checkedCast(proxy, facetOrContext=None, context=None):
            return _M_Example.PrinterFactoryPrx.ice_checkedCast(proxy, '::Example::PrinterFactory', facetOrContext, context)

        @staticmethod
        def uncheckedCast(proxy, facet=None):
            return _M_Example.PrinterFactoryPrx.ice_uncheckedCast(proxy, facet)

        @staticmethod
        def ice_staticId():
            return '::Example::PrinterFactory'
    _M_Example._t_PrinterFactoryPrx = IcePy.defineProxy('::Example::PrinterFactory', PrinterFactoryPrx)

    _M_Example.PrinterFactoryPrx = PrinterFactoryPrx
    del PrinterFactoryPrx

    _M_Example.PrinterFactory = Ice.createTempClass()
    class PrinterFactory(Ice.Object):

        def ice_ids(self, current=None):
            return ('::Example::PrinterFactory', '::Ice::Object')

        def ice_id(self, current=None):
            return '::Example::PrinterFactory'

        @staticmethod
        def ice_staticId():
            return '::Example::PrinterFactory'

        def create(self, name, current=None):
            raise NotImplementedError("servant method 'create' not implemented")

        def __str__(self):
            return IcePy.stringify(self, _M_Example._t_PrinterFactoryDisp)

        __repr__ = __str__

    _M_Example._t_PrinterFactoryDisp = IcePy.defineClass('::Example::PrinterFactory', PrinterFactory, (), None, ())
    PrinterFactory._ice_type = _M_Example._t_PrinterFactoryDisp

    PrinterFactory._op_create = IcePy.Operation('create', Ice.OperationMode.Normal, Ice.OperationMode.Normal, False, None, (), (((), IcePy._t_string, False, 0),), (), ((), _M_Example._t_PrinterPrx, False, 0), ())

    _M_Example.PrinterFactory = PrinterFactory
    del PrinterFactory

# End of module Example
